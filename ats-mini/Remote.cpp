...
static bool remoteSetFreq(Stream* stream)
{
  stream->print('F');
  uint32_t freq = (uint32_t)remoteReadInteger(stream);
  char mode[4] = {0};

  if(stream->peek() == ',') {
    remoteReadChar(stream); // consume ','
    remoteReadString(stream, mode, 4);
  }
  if(!expectNewline(stream))
    return remoteShowError(stream, "Expected newline");
  ...
  // Resolve mode index if specified
  uint8_t modeIdx = 0xFF;
  if(mode[0]) {
    for(int i = 0; i < getTotalModes(); i++) {
      if(strcmp(bandModeDesc[i], mode) == 0) { modeIdx = i; break; }
    }
    if(modeIdx == 0xFF)
      return remoteShowError(stream, "No such mode");
  }
  ...
  if(bandMatch == 0xFF)
    return remoteShowError(stream, "Frequency out of range");

  uint8_t m = (modeIdx != 0xFF) ? modeIdx : bands[bandMatch].bandMode;
  Memory mem;
  mem.freq = freq;
  mem.mode = m;
  mem.band = bandMatch;
...
void remotePrintStatus(Stream* stream, RemoteState* state)
{
  ...
  // AVC setting (AM/SSB only; 0 in FM mode)
  int8_t remoteAvc = (currentMode == FM) ? 0 : (isSSB() ? SsbAvcIdx : AmAvcIdx);

  // Stereo pilot detection (FM only)
  uint8_t remoteStereo = (currentMode == FM) ? (rx.getCurrentPilot() ? 1 : 0) : 0;

  // Remote serial
  stream->printf("%u,%u,%d,%d,%s,%s,%s,%s,%hu,%hu,%hu,%hu,%hu,%.2f,%hu,%d,%u\r\n",
                VER_APP,
                currentFrequency,
                currentBFO,
...
int remoteDoCommand(Stream* stream, RemoteState* state, char key)
{
  ...
  switch(key)
  {
    ...
    case '#':
      ...
      break;
    case 'F':
      if (remoteSetFreq(stream))
        event |= REMOTE_PREFS;
      break;
    case 'Q':
      muteOn(MUTE_MAIN, !muteOn(MUTE_MAIN));
      stream->println(muteOn(MUTE_MAIN) ? "Muted" : "Unmuted");
      break;
    case 'N':  // AVC up
      doAvc(1);
      event |= REMOTE_PREFS;
